...
def process_extraction(document_id, schema, api_key):
    ...
    if result["needs_review"]:
        ...
        print("⚠️ Review needed - direct team to: https://app.documind.cloud/review")
...
<Tab title="Manual Check">
  **Best for**: Batch processing, scheduled jobs
  ...
  for doc in daily_documents:
      requests.post(
          f"https://api.documind.cloud/api/v1/extract/{doc.id}",
          headers={"X-API-Key": API_KEY, "Content-Type": "application/json"},
...
<Card title="List Pending Reviews" icon="list" href="/api-reference/pending-reviews">
