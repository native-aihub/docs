...
```json
{
  ...
  // or "google-gemini-2.5-flash"             // 4 credits/page
  // or "google-gemini-3-flash"              // 6 credits/page
}
```
...
|----------------|---------------|
| Basic (`qwen-3-vl`) | 2 credits |
| Basic (`google-gemini-2.5-flash`) | 4 credits |
| Basic (`google-gemini-3-flash`) | 6 credits |
| VLM | 10 credits |
| Advanced | 15 credits |
...
- `extractions:write` - Create and update extractions
- `api_keys:read` - List API keys
- `api_keys:write` - Create and manage API keys
- `usage:read` - View usage and credits
