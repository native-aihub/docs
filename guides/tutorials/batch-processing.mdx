...
```python
# Submit uploaded documents as one async batch job
...
# Poll until status is completed, failed, or partial_failed
batch = requests.get(f"{BASE_URL}/batch/{batch_id}", headers=headers).json()
```
